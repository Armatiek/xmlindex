...
public class FollowingSiblingAxisIterator extends SearchResultsForwardAxisIterator  {
  ...
  protected void addAxisClauses(Builder queryBuilder) {
    ...
    queryBuilder.add(new BooleanClause(getExactQuery(Definitions.FIELDNAME_PARENT, node.parent), BooleanClause.Occur.FILTER));
    queryBuilder.add(new BooleanClause(getRangeQuery(Definitions.FIELDNAME_LEFT, left, Definitions.MAX_LONG), BooleanClause.Occur.FILTER));
  }
