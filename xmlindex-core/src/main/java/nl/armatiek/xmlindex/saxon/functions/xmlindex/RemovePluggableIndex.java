 ...
 */
public class RemovePluggableIndex extends ExtensionFunctionDefinition {
  
  private static final StructuredQName qName = 
      new StructuredQName("", Definitions.NAMESPACE_EXT_FUNCTIONS, "remove-pluggable-index");
  ...
  private static class RemovePluggableIndexCall extends XMLIndexExtensionFunctionCall {
    ...
    public BooleanValue call(XPathContext context, Sequence[] arguments) throws XPathException {            
      try {
        ...
        PluggableIndex pluggableIndex = index.getConfiguration().getPluggableIndexConfig().get(className);
        if (pluggableIndex == null)
          throw new XPathException("Pluggable index \"" + className + "\" does not exists");
        index.getConfiguration().getPluggableIndexConfig().remove(pluggableIndex);
        if (reindex)
