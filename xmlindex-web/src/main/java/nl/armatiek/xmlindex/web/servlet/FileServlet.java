...
public abstract class FileServlet extends HttpServlet {
  ...
  private List<Range> getRanges(HttpServletRequest request, Resource resource) {
    ...
    if (ifRange != null && !ifRange.equals(resource.eTag)) {
      try {
        ...
        logger.warn("If-Range header is invalid. Let's just return full file then.", ifRangeHeaderIsInvalid);
        return ranges;
      }
    }
  ...
  private String setContentHeaders(HttpServletRequest request, HttpServletResponse response, Resource resource, List<Range> ranges) {
    ...
    response.setHeader("Content-Disposition", format(CONTENT_DISPOSITION_HEADER, (attachment ? "attachment" : "inline"), encodeURI(filename)));
   ...
   * @since 2.2
   */
  public void setCacheHeaders(HttpServletResponse response, long expires) {
    if (expires > 0) {
   ...
   * @since 1.4
   */
  public boolean startsWithOneOf(String string, String... prefixes) {
   ...
   */
  @SafeVarargs
  public final <T> T coalesce(T... objects) {
